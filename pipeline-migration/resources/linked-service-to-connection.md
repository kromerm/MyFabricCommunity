...
> **For Self-Hosted Integration Runtime (SHIR) backed linked services**: see [pipeline-gotchas.md § PG5](pipeline-gotchas.md) — you must configure an on-premises data gateway in Fabric.
...
az rest --method GET \
  ...
  --query "value[].{name:displayName, type:connectorId, id:id}" -o table
```
...
Connection creation requires `connectivityType`, `displayName`, `connectorId`, and `connectionDetails` (credentials and endpoint).
...
az rest --method POST \
  ...
  --body '{
    ...
    "connectorId": "AzureDataLakeStorage",
    "connectionDetails": {
...
> For ServicePrincipal auth, replace the `credentialDetails` block with `"credentialType": "ServicePrincipal"` and supply `tenantId`, `clientId`, and `clientSecret` via `--body @body.json` (see the security note below — never pass `clientSecret` inline).

> **Security note**: Never pass `clientSecret` as a literal string in a shell command — it will appear in shell history and process logs. Prefer one of:
> - **WorkspaceIdentity** (managed identity) — set `"credentialType": "WorkspaceIdentity"` and omit `clientSecret` entirely. This is the recommended approach when the Fabric workspace has a managed identity assigned.
...
>   ```bash
>   printf '{"credentialType":"ServicePrincipal","servicePrincipalId":"...","clientSecret":"%s","tenantId":"..."}' "$MY_SP_SECRET" > /tmp/conn_body.json
...
>   ```powershell
>   $body = "{`"credentialType`":`"ServicePrincipal`",`"servicePrincipalId`":`"...`",`"clientSecret`":`"$env:MY_SP_SECRET`",`"tenantId`":`"...`"}"
...
>     --body "{\"credentialType\":\"ServicePrincipal\",\"clientSecret\":\"$MY_SP_SECRET\",\"tenantId\":\"...\"}"
>   ```
...
> **SHIR → Gateway**: The Fabric portal path is: **Settings → Manage connections and gateways → On-premises data gateways**. Install the on-premises data gateway and register it before creating SHIR-backed connections in Fabric. See [pipeline-gotchas.md § PG5](pipeline-gotchas.md) for the full procedure.
