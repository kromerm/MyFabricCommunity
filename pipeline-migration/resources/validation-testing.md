...
| **V2** | Pipeline runs without error | Status = `Succeeded` (Fabric pipeline run terminal status) |
    ...
    for _ in range(max_polls):
        ...
        if status == "Succeeded":
            ...
            if result_url:
                ...
                return requests.get(result_url, headers=headers, timeout=http_timeout).json()
            return state
        elif status == "Failed":
...
def verify_notebook_activity_output(run_result: dict, activity_name: str, expected_key: str = None):
    ...
    # This checks the top-level run status as a proxy
...
def check_copy_activity_success(run_result: dict) -> bool:
    ...
    """
    return run_result.get("status") == "Succeeded"
```
...
class PipelineValidationResult:
    ...
    def passed(self) -> bool:
        critical = [self.v1_definition_ok, self.v2_run_status == "Succeeded"]
    ...
    def summary_row(self) -> dict:
        return {
            ...
            "V2 Run": "✅" if self.v2_run_status == "Succeeded" else f"❌ {self.v2_run_status}",
