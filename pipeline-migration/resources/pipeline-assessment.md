...
# Activities that drop in cleanly with no required edits.
COMPATIBLE_ACTIVITIES = {
    "Copy", "Lookup", "GetMetadata", "Delete",          # dataset-inlining needed
...
class ActivityRecord:
    name: str
    activity_type: str
    classification: str   # "compatible" | "rewrite" | "parked"
    note: str = ""
...
def classify_activity(activity_type: str) -> tuple[str, str]:
    ...
    if activity_type in PARKED_ACTIVITIES:
        ...
        # Compatible activity type, but still needs targeted edits. Returned as
        # 'compatible' so the count buckets stay correct, with a non-empty note
        # so the assessment report surfaces the required work (workspaceId,
        # GUID swap, etc.) instead of marking it as zero-action.
        return "compatible", COMPATIBLE_WITH_CHANGES[activity_type]
...
def generate_assessment_report(data: dict, output_path: Optional[str] = None) -> str:
    ...
    clean_pipelines = [a for a in assessments.values() if a.rewrite_count == 0 and a.parked_count == 0]
    ...
    lines = [
        ...
        f"| 🟢 Migrate cleanly (no rewrites, no blockers) | {len(clean_pipelines)} |",
